Feature: NLS Reference Page verification
...
Background:
      ...
      Then Navigate to NLS Reference PAGE
      
	@Scenario1ipl
  Scenario: Verify NLS Reference page having all the fields all list_grid_card views
    And Check view having all the below fields
		| Function  |   Reference key  |   NLS Reference text |  Start Date  | End date  |  Status  |
  ...
  Then add new record in NLS Reference page with below values and check db "<Function>""<Ref_key>""<NLS_ref_text>""<Effective_Date>""<Delete_Date>""<QUERY>"
  ...
  Examples:
	| Function  |   Ref_key  | NLS_ref_text  |  Effective_Date  | Delete_Date  | QUERY  |
