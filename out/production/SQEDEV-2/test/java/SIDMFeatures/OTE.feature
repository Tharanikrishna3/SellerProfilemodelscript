...
Background:
      ...
      Then Navigate to OTE Page
      
	@Scenario1ipl
  Scenario: Verify OTE page having all the fields all list_grid_card views
    And Check view having all the below fields
		| OTE group |   	OTE name  |   Start Date  |  End date  |  Status  |
    ...
    @Scenario2ipl
  Scenario Outline: Verify Add Edit working fine in OTE page
  Then add new record in OTE  page with below values and check db "<OTE_grp>""<OTE_name>""<Effective_Date>""<Delete_Date>""<QUERY>"
  Then Moving to back to list 
	Then Search for ipl_status "2022 1H"
	Then check edit function is working fine for IPL status
  Examples:
	| OTE_grp  |   OTE_name  |  Effective_Date  | Delete_Date  | QUERY  |
	| K | 06 | 2022-01-01  |  2022-06-30   | SELECT * FROM DBLFMSXR.FMST_H_IPL_STATUS |
	
	@Scenario3ipl
  Scenario Outline: Verify Add Edit and cancel working fine in OTE  PAGE
  Then add new record in OTE  page with below values and check db "<OTE_grp>""<OTE_name>""<Effective_Date>""<Delete_Date>""<QUERY>"
  Then Moving to back to list  
  Then Search for ipl_status "2022 1H"
