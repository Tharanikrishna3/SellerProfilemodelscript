Feature: Calc Basis Page verification

Background:
      Given User navigates to Home page ""
    	Then Login into application with the id "anacst@br.ibm.com","123"
      Then Navigate to Calc Basis Page
      
	@Scenario1sm
  Scenario: Verify Calc Basis Page page having all the fields all list_grid_card views
    And Check view having all the below fields
		| Calculation basis code  |   Calculation basis name  |   Start date  |  	End date  |  Status  |
    ...
    @Scenario2sm
  Scenario Outline: Verify Add Edit and cancel working fine in Calc Basis Page
  Then add new record in Calc Basis page with below values "<Calc_basis_code>""<Calc_basis_name>""<Effective_Date>""<Delete_Date>"
  ...
  Examples:
	| Calc_basis_code  |   Calc_basis_name  |  Effective_Date  | Delete_Date  |
